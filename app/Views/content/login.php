<section id="login">
    <div class="container-fluid bg-login w-100 d-flex justify-content-center align-items-center vh-100 position-relative">
        <div class="position-absolute m-3 top-0 end-0 p-2 bg-secondary rounded-5 d-flex justify-content-around align-items-center gap-4">
            ...
            <a href="/" class="btn btn-warning rounded-5 ">
                <i class="bi bi-person-circle me-2"></i>Guest
            </a>
        </div>
        <div class="w-50 form-login p-5 rounded-5 border border-4 shadow-lg d-flex justify-content-center align-items-center flex-column">
            <form method="post" action="/login" class="form-floating needs-validation w-100">
                <?= csrf_field() ?> <!-- penting kalau CSRF aktif di config -->
                <h1 class="color-hijau text-center fw-bold">FORM LOGIN ADMIN</h1>
